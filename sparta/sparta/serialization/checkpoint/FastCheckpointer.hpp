...
namespace sparta::serialization::checkpoint
{
    ...
    public:
        ...
        void traceValue(std::ostream& o, chkpt_id_t id, const ArchData* container, uint32_t offset, uint32_t size) override {
            store_.traceValue(o, id, container, offset, size);
        }

        ////////////////////////////////////////////////////////////////////////
        //! @}

        using Checkpointer<BackingStore>::getHead;
        using Checkpointer<BackingStore>::getArchDatas;
        using Checkpointer<BackingStore>::getCurrentTick;
        using Checkpointer<BackingStore>::getRoot;
    ...
    private:
        ...
        void createHead_() override {
            ...
            std::unique_ptr<checkpoint_type> chkpt(dcp);
            store_.insert(std::move(chkpt));
            setHead_(dcp);
            num_alive_checkpoints_++;
            num_alive_snapshots_++;
            setCurrent_(dcp);
        ...
        using Checkpointer<BackingStore>::getCurrent_;
        using Checkpointer<BackingStore>::setCurrent_;
        using Checkpointer<BackingStore>::getHead_;
        using Checkpointer<BackingStore>::setHead_;
        using Checkpointer<BackingStore>::store_;
        using Checkpointer<BackingStore>::sched_;
